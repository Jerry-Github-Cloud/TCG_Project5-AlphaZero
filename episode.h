...
public:
	...
	bool apply_action(action move, std::map<size_t, size_t> policy_labels={}) {
		board::reward reward = move.apply(state());
		if (reward != board::legal) return false;
		ep_moves.emplace_back(move, reward, millisec() - ep_time, policy_labels);
		ep_score += reward;
		return true;
	}
...
protected:
	...
	struct move {
		...
		move(action code = {}, board::reward reward = 0, time_t time = 0, std::map<size_t, size_t> p = {}) : code(code), reward(reward), policy_labels(p) {}
		...
		friend std::ostream& operator <<(std::ostream& out, const move& m) {
			...
			out << "C[{";
			size_t i = 0;
			for (auto p : m.policy_labels) { out << p.first << ":" << p.second << ","; }
			out << "}]";
			return out;
		}
