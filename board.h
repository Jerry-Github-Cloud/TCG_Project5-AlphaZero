...
public:
	...
	bool has_legal_move(piece_type who) const {
		for (uint x = 0; x < size_x; x++) {
			for (uint y = 0; y < size_y; y++) {
				board after = *this;
				if (after.place(x, y, who) == legal) { return true; }
			}
		}
		return false;
	}
	std::vector<point> get_legal_move(piece_type who) {
		std::vector<point> legal_move;
		for (uint x = 0; x < size_x; x++) {
			for (uint y = 0; y < size_y; y++) {
				board after = *this;
				if (after.place(x, y, who) == legal) {
					legal_move.push_back(point(x, y));
				}
			}
		}
		return legal_move;
	}
	std::vector<float> observation_tensor() const {
		std::vector<float> tensor;
		tensor.reserve(10 * 9 * 9);
		// 1 black 
		for (uint x = 0; x < size_x; x++) {
			for (uint y = 0; y < size_y; y++) {
				tensor.push_back(static_cast<float>(stone[x][y] == black));
			}
		}
		// 2 white
		for (uint x = 0; x < size_x; x++) {
			for (uint y = 0; y < size_y; y++) {
				tensor.push_back(static_cast<float>(stone[x][y] == white));
			}
		}
		// 3 empty
		for (uint x = 0; x < size_x; x++) {
			for (uint y = 0; y < size_y; y++) {
				tensor.push_back(static_cast<float>(stone[x][y] == empty));
			}
		}
		// 4 hollow
		for (uint x = 0; x < size_x; x++) {
			for (uint y = 0; y < size_y; y++) {
				tensor.push_back(static_cast<float>(stone[x][y] == hollow));
			}
		}
		// 5 who take turn
		for (uint x = 0; x < size_x; x++) {
			for (uint y = 0; y < size_y; y++) {
				tensor.push_back(static_cast<float>(attr.who_take_turns));
			}
		}
		// 6 black one liberties
		for (uint x = 0; x < size_x; x++) {
			for (uint y = 0; y < size_y; y++) {
				tensor.push_back(static_cast<float>(check_liberty(x, y, black) == 1));
			}
		}
		// 7 white one liberties
		for (uint x = 0; x < size_x; x++) {
			for (uint y = 0; y < size_y; y++) {
				tensor.push_back(static_cast<float>(check_liberty(x, y, white) == 1));
			}
		}
		// 8 black two liberties
		for (uint x = 0; x < size_x; x++) {
			for (uint y = 0; y < size_y; y++) {
				tensor.push_back(static_cast<float>(check_liberty(x, y, black) == 2));
			}
		}
		// 9 white two liberties
		for (uint x = 0; x < size_x; x++) {
			for (uint y = 0; y < size_y; y++) {
				tensor.push_back(static_cast<float>(check_liberty(x, y, white) == 2));
			}
		}
		// 10 Both black and white cannot take this move
		for (uint x = 0; x < size_x; x++) {
			for (uint y = 0; y < size_y; y++) {
				tensor.push_back(static_cast<float>((check_liberty(x, y, black) == 1) and (check_liberty(x, y, white) == 1)));
			}
		}
		return tensor;
	}
